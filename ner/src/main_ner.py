...
class RunConfig:
    ...
    device:str = "cuda:0"
    output_entity_label:str = "Entity"
...
def brat_annotation_to_instances(doc:BRAT.Document, tokenizer:NER.TokenizerInterface, max_length:int, stride:int, rstrip_text:bool, doc_name:str="(not-given)") -> tuple[list[NER.NERInstance], NER.NERInstance]:
    ...
    doc_instances, full_doc_instance = NER.NERInstance.build(
        ...
        truncation=NER.NERTruncationScheme.SPLIT, max_length=max_length, stride=stride, fit_to_token=True, add_split_idx_to_id=False, return_non_truncated=True,
    )
...
def build_dataset(instances:list[NER.NERInstance], device:str) -> SelectiveDataset:
    fields = [
        ...
        {"name":"label", "mapping":lambda x:x.get_sequence_label(tagging_scheme=NER.NERTaggingScheme.BILOU, label_scheme=NER.NERLabelScheme.SPAN_ONLY), "dtype":torch.long, "device":device, "padding":True, "padding_value":0},
...
class Meter:
    ...
    def update(self, gold_spans:list[NER.NERSpan], pred_spans:list[NER.NERSpan], doc_id):
        self.doc_id_to_gold_spans[doc_id] |= {(span.start, span.end) for span in gold_spans}
        self.doc_id_to_pred_spans[doc_id] |= {(span.start, span.end) for span in pred_spans}
    ...
    def output_predictions_as_ann_to_dir(self, dir_path:str|pathlib.Path, entity_label:str="Entity", doc_id_to_text_func:Optional[Callable[[str], str]]=None):
        if type(dir_path) is str:
            dir_path = pathlib.Path(dir_path)
...
# %%
def run_epoch(
    model:M.Model, dataset:SelectiveDataset, doc_name_to_full_instance:dict[str,NER.NERInstance], boundary_exclusion_size:int, process_batch_size:int, is_training:bool,
        ...
        for minibatches in to_chunk(dataloader_pbar, num_accumulate_steps):
            for minibatch in minibatches:
                ...
                for instance, step_logits, step_token_len in zip(minibatch["instance"], logits.detach().cpu().numpy(), minibatch["token_len"]):
                    ...
                    step_preds:list[NER.NERSpan] = NER.viterbi_decode(step_logits, tagging_scheme=NER.NERTaggingScheme.BILOU, label_scheme=NER.NERLabelScheme.SPAN_ONLY, scalar_logit_for_token_level=True, as_spans=True)
                    ...
                    meter_by_union.update(gold_spans=instance.recover_split_offset_of_char_spans(instance.spans), pred_spans=step_preds, doc_id=instance.id)
    ...
    with closing_tqdm(doc_name_to_split_instance_and_logits_pairs.items(), total=len(doc_name_to_split_instance_and_logits_pairs), desc=ensemble_desc_prefix) as pbar:
        for doc_name, split_and_logits_pairs in pbar:
            ...
            ensembled_preds:list[NER.NERSpan] = NER.viterbi_decode(ensembled_logits, tagging_scheme=NER.NERTaggingScheme.BILOU, label_scheme=NER.NERLabelScheme.SPAN_ONLY, scalar_logit_for_token_level=True, as_spans=True)
            ensembled_preds:list[NER.NERSpan] = non_split_instance.decode_token_span_to_char_span(ensembled_preds, strip=True, offset_to_skip_forward_special_tokens=True)
            ensembled_preds = [span for span in ensembled_preds if span.start < span.end]
            meter_by_ensemble.update(gold_spans=non_split_instance.spans, pred_spans=ensembled_preds, doc_id=doc_name)
...
model_config = M.ModelConfig(bert_name=run_config.bert_name, dropout_output=run_config.dropout_output)
...
if run_config.mode == "train":
    ...
    train_dataset = build_dataset(train_instances, device=run_config.device)
    dev_dataset = build_dataset(dev_instances, device=run_config.device)
    ...
    for epoch in range(1,run_config.max_epoch+1):
        ...
        train_loss, train_meter_by_union, train_meter_by_ensemble = run_epoch(
            model=model, dataset=train_dataset, doc_name_to_full_instance=train_doc_name_to_full_instance, boundary_exclusion_size=run_config.boundary_exclusion_size, process_batch_size=run_config.process_batch_size, is_training=True,
        ...
        dev_loss, dev_meter_by_union, dev_meter_by_ensemble = run_epoch(
            model=model, dataset=dev_dataset, doc_name_to_full_instance=dev_doc_name_to_full_instance, boundary_exclusion_size=run_config.boundary_exclusion_size, process_batch_size=run_config.process_batch_size, is_training=False,
        )
        ...
        if best_scores.get("dev/f", -1.0) < dev_f_score:
            ...
            if save_dir_path is not None:
                ...
                dev_meter_by_ensemble.output_predictions_as_ann_to_dir(save_dir_path/"preds_ensemble", entity_label=run_config.output_entity_label, doc_id_to_text_func=lambda x:dev_doc_name_to_full_instance[x].text)
                has_saved = True
        if run_config.use_average:
            ...
            if best_scores.get("dev-ave/f", -1.0) < dev_ave_f_score:
                ...
                if save_dir_path is not None:
                    ...
                    dev_ave_meter_by_ensemble.output_predictions_as_ann_to_dir(save_dir_path/"preds-ema_ensemble", entity_label=run_config.output_entity_label, doc_id_to_text_func=lambda x:dev_doc_name_to_full_instance[x].text)
                    has_saved = True
        if has_saved:
    ...
    val_dataset = build_dataset(val_instances, device=run_config.device)
    ...
    val_loss, val_meter_by_union, val_meter_by_ensemble = run_epoch(
        model=model, dataset=val_dataset, doc_name_to_full_instance=val_doc_name_to_full_instance, boundary_exclusion_size=run_config.boundary_exclusion_size, process_batch_size=run_config.process_batch_size, is_training=False,
    )
    ...
    if run_config.eval_output_dir is not None:
        ...
        val_meter_by_union.output_predictions_as_ann_to_dir(eval_output_dir_path/"preds_union", entity_label=run_config.output_entity_label, doc_id_to_text_func=lambda x:val_doc_name_to_full_instance[x].text)
        val_meter_by_ensemble.output_predictions_as_ann_to_dir(eval_output_dir_path/"preds_ensemble", entity_label=run_config.output_entity_label, doc_id_to_text_func=lambda x:val_doc_name_to_full_instance[x].text)
