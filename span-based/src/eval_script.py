...
class ClinicalConcept(object):
    ...
    def span_matches(self, other, mode="strict"):
        ...
        if mode == "strict":
            ...
            if (self.end > other.start and self.start < other.end) or (self.start < other.end and other.start < self.end):
                return True
        return False
...
class Attribute(object):
    ...
    def equals(self, other, mode="strict"):
        """Return whether the current tag is equal to the one provided."""
        assert mode in ("strict", "lenient")
        if self.arg.equals(other.arg, mode) and self.arg.equals(other.arg, mode) and self.rtype == other.rtype and self.rval == other.rval:
            return True
        return False
        ...
        with open(self.path) as annotation_file:
            ...
            for line_num, line in enumerate(lines):
                if line.strip().startswith("T"):
                    ...
                    annotations["tags"]["D_" + tag_id] = ClinicalConcept("D_" + tag_id, tag_start, tag_end, "Drug", tag_text)
            ...
            for line_num, line in enumerate(lines):
                if line.strip().startswith("A"):
                    ...
                    annotations["attributes"][attr_id] = Attribute(attr_id, arg1, attr_type, attr_val)
...
def evaluate(corpora, mode="strict", verbose=False):
    ...
    print("{:20}  {:6}  {:6}  {:6}    {:6}  {:6}  {:6}".format("", "Prec.", "Rec.", "F(b=1)", "Prec.", "Rec.", "F(b=1)"))
...
if __name__ == "__main__":
    ...
    print("evaluate folder: {}".format(args.folder2))
